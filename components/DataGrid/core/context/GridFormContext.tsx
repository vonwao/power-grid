...
/**
 * Grid form context type definition
 */
interface GridFormContextType {
  // Form instances for each row
  getFormMethods: (rowId: any) => FormMethods | undefined;
  
  // Row management
  startEditingRow: (rowId: any, field: string) => void;
  stopEditingRow: (rowId: any) => void;
  
  // Cell management
  startEditingCell: (rowId: any, field: string) => void;
  stopEditingCell: () => void;
  getCurrentCell: () => { rowId: any; field: string } | undefined;
  
  // Row state
  isRowEditing: (rowId: any) => boolean;
  isRowDirty: (rowId: any) => boolean;
  isFieldDirty: (rowId: any, field: string) => boolean;
  getRowErrors: (rowId: any) => Record<string, FieldError> | undefined;
  
  // Row-level validation
  validateRow: (rowId: any) => Promise<boolean>;
  
  // Value updates
  updateCellValue: (rowId: any, field: string, value: any) => void;
 ...
 */
const createFormInstance = (defaultValues: Record<string, any>, columns: CoreColumnConfig[]): FormMethods => {
  try {
    // Create a form state
...
export function GridFormProvider({
  ...
  const [currentCell, setCurrentCell] = useState<{ rowId: any; field: string }>();
  
  // Add state to track pending changes
  const [pendingChanges, setPendingChanges] = useState<Map<any, Record<string, any>>>(new Map());
  
  // Use a ref to store form instances to avoid re-renders when they change
  const formInstancesRef = useRef<Map<any, FormMethods>>(new Map());
  
  // Track original data for each row being edited
  const originalDataRef = useRef<Map<any, any>>(new Map());
  
  // Track rows that were added (not in the original data)
  const addedRowsRef = useRef<Set<any>>(new Set());
  ...
  const getFormMethods = useCallback((rowId: any): FormMethods | undefined => {
    try {
  ...
  // Start editing a row
  const startEditingRow = useCallback((rowId: any, field: string) => {
    try {
  ...
  // Stop editing a row
  const stopEditingRow = useCallback((rowId: any) => {
    try {
  ...
  const startEditingCell = useCallback((rowId: any, field: string) => {
    try {
  ...
  const isRowEditing = useCallback((rowId: any) => {
    try {
  ...
  const isRowDirty = useCallback((rowId: any) => {
    try {
  ...
  const isFieldDirty = useCallback((rowId: any, field: string) => {
    try {
      // Use our explicit pendingChanges tracking
  ...
  const getRowErrors = useCallback((rowId: any) => {
    try {
  ...
  // Validate a row
  const validateRow = useCallback(async (rowId: any): Promise<boolean> => {
    try {
      const form = formInstancesRef.current.get(rowId);
      if (!form) return true;
  ...
  // Update a cell value
  const updateCellValue = useCallback((rowId: any, field: string, value: any) => {
    try {
      const form = formInstancesRef.current.get(rowId);
      if (form) {
  ...
  const saveChanges = useCallback(() => {
    try {
      ...
      const edits: Array<{ id: any, changes: any }> = [];
...
/**
 * Hook to use the grid form context
 */
